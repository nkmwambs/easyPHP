...
class Finance_Model extends E_Model
{
    ...
    public function showVoucher($VNumber,$icpNo){
        ...
        while ($row = mysql_fetch_object($getQry)) {
            $rst[]=$row;
        }
        return $rst;
    }
 public function getVoucherTableFields($tbl){
                        ...
                        while($rows=mysql_fetch_object($q2)){
    ...
    try{
        if($rst){
            ...
            throw new customException("MySQL Query not Valid!");
        }
  ...
  // $counter = 0;
    ...
    do {
       // if ($data[0]) {
            ...
            mysql_query("INSERT INTO fundsschedule (KENumber, AccountCode,AccountDescription,Amount,Month) VALUES
                (
                    '".addslashes($data[0])."',
                    '".addslashes($data[1])."',
                    '".addslashes($data[2])."',
    ...
    elseif($lists=="projectsDetails")
    {
	    do {
       // if ($data[0]) {
            mysql_query("INSERT INTO projectsdetails (KENumber, ProjectName,AccNumber,AccountName,Bank,Branch,BranchCode,PFName,Cluster,PEmail) VALUES
    ...
    elseif($lists=="specialgifts")
    {
	    do {
       // if ($data[0]) {
            mysql_query("INSERT INTO specialgifts (KENumber,ChildNumber,Amount,Instructions,Month) VALUES
