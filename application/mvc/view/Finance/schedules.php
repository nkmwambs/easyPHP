...
echo "<button onclick='addScheduleRow();' id='btnAddRow' style='display:none;'>Add Row</button><button style='display:none;' id='btnPostSchedule' onclick='postSchedule(\"frmSchedule\");'>Post Schedule</button><button id='btnDelRow' style='display:none;' onclick='delRow(\"tblSchedule\");'>Delete Row</button>";
echo "<div id='scheduleview'>";

echo "<form id='frmSchedule'>";
echo "<table id='tblSchedule' style='white-space:nowrap;'>";
echo "<caption style='text-align:left;'><i><b>".Resources::img("inform.png")." Choose Account and Click on the ".Resources::img("go.png")." button below to view a Schedule (<span style='color:red;'>You can only View and Edit Items not Approved</span>)</b></i></caption>";
echo "<tr><th colspan='2'>Account:<select name='AccNo' id='AccNo'><option value=''>Select Account ... </option>";
        foreach ($data[1] as $ac):
            echo "<option value='".$ac->AccNo."'>".$ac->AccText." - ".$ac->AccName." </option>";
        endforeach;
    echo "</select>".Resources::img("go.png",array("title"=>"Go","onclick"=>'checkSchedule();',"style"=>"cursor:pointer;"))."</th><th colspan='2'>".Resources::a_href("Finance/schedules","Reset")."</th><th colspan='2'>FY:</th><th colspan='2'><input type='text' value='".$data[0]."' id='fy' name='fy' style='width:40px;' readonly/></th><th colspan='13'></th></tr>";
