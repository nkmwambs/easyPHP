...
echo "<br><br><hr>";
echo "<button onclick='viewPlanSummary();'>View Budget Summary</button>"." "."<button onclick='viewAllSchedules();'>View All Schedules</button>".a_tag("Finance/schedules","<button>New Budget Schedules</button>",array("id"=>"btnNewSchedule","style"=>"display:none"))."<br><br>";
echo "<hr><br>";
$months_arr = get_month_number_array();
...
echo "<caption style='text-align:left;'><i><b>".img_tag("inform.png")." Choose Account and Click on the ".img_tag("go.png")." button below to view a Schedule</b></i></caption>";
echo "<tr><th colspan='2'>Account:<select name='AccNo' id='AccNo'><option value=''>Select Account ... </option>";
        ...
        endforeach;
    echo "</select>".img_tag("go.png",array("title"=>"Go","onclick"=>'checkSchedule();',"style"=>"cursor:pointer;"))."</th><th colspan='2'>".a_tag("Finance/schedules","Reset")."</th><th colspan='2'>FY:</th><th colspan='2'><input type='text' value='".$data[0]."' id='fy' name='fy' style='width:30px;' readonly/></th><th colspan='13'></th></tr>";
...
foreach (get_month_number_array() as $key=>$value):
