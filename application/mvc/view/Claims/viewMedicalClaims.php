...
echo "<div id='control' align='center' style='border:2px lightgray groove;padding:5px;'>  | Filter | From: <input style='width:100px;'  type='text' id='frmDate'  value='".$data['fromdate']."'/> To: <input style='width:100px;'  type='text' id='toDate'  value='".$data['todate']."'/> State: <select id='rmkSelect'><option value=''>Select State ...</option><option value='0'>New Claim</option><option value='2'>Checked By PF</option><option value='1'>Rejected By PF</option><option value='4'>Processed By HS</option><option value='3'>Rejected By HS</option><option value='7'>Paid</option></select>".Resources::img("go.png",  array("title"=>"Go","id"=>"btnselectClaims","onclick"=>'selectClaims()',"style"=>'cursor:pointer;'))."</div>";
    ...
    echo "<div id='control' align='center' style='border:2px lightgray groove;padding:5px;'> | Filter | From: <input type='text' style='width:100px;' id='frmDate' value='".$data['fromdate']."'/> To: <input  style='width:100px;' type='text' id='toDate' value='".$data['todate']."'/> State: <select  id='rmkSelect'><option  value=''>Select State ...</option><option value='0'>New Claim</option><option value='2'>Checked By PF</option><option value='1'>Rejected By PF</option><option value='4'>Processed By HS</option><option value='3'>Rejected By HS</option><option value='7'>Paid</option></select>".Resources::img("go.png",  array("title"=>"Go","id"=>"btnselectClaims","onclick"=>'selectClaims();',"style"=>'cursor:pointer;'));
	if(Resources::session()->userlevel==='5'){
		echo  "| ".Resources::img("pay.png",array("title"=>"Pay","style"=>"cursor:pointer;","onclick"=>"paymedicalclaims()"))." Count Of Unpaid Claims: <b>".$data['cnt']."</b>";
	}
	echo "</div>";
}
...
if(Resources::session()->userlevel==='1'){
	echo "<span style='font-weight:bold;'>Download Medical Claims: </span><a href='".HOST_NAME."/easyPHP/application/mvc/docs/exceldownloads/medicalclaims.php?from=".strtotime($data['fromdate'])."&to=".strtotime($data['todate'])."&rmk=".$data['rmk']."&proNo=".Resources::session()->fname."'>".Resources::img("excel.png")."</a>";
}elseif(Resources::session()->userlevel==='2'){
	echo "<span style='font-weight:bold;'>Download Medical Claims: </span><a href='".HOST_NAME."/easyPHP/application/mvc/docs/exceldownloads/medicalclaims.php?from=".strtotime($data['fromdate'])."&to=".strtotime($data['todate'])."&rmk=".$data['rmk']."&cluster=".Resources::session()->cname."'>".Resources::img("excel.png")."</a>";
}else{
...
$rmk_arr = array("Unchecked By PF","Rejected By PF","Checked By PF","Rejected By HS","Processed By HS","Rejected By PD","Approved By PD/ Unpaid","Paid");
...
echo "<caption style='text-align:left;'><b>Claim Status: ".$status."</b></caption>";
    echo "<thead>";
    echo "<col><col><col><col><col><col><col><col><col><col><col><col><col><col><col><col><col><col><col><col><col>";
    ...
    $waive="";
       foreach($data['claims'] as $elem):
           ...
           if($elem->rmks==='1'&&($_SESSION['userlevel']==='1')){$rmk=Resources::img("edit.png",  array("style"=>"border:2px red solid;margin:2px;","title"=>"Edit","onclick"=>"editrejectedclaim(\"".$elem->rec."\",this)"))." ".Resources::img("uncheck.png",array("style"=>"border:2px green solid;margin:2px;","title"=>"Delete Claim","onclick"=>"deleteClaim(this,\"$elem->rec\");"));}
           ...
           if($elem->rmks==='3'&&($_SESSION['userlevel']!=='5')){
           		$rmk=$rmk=Resources::img("unprocessed.png",array("style"=>"border:2px green solid;margin:2px;","title"=>"Rejected By HS"));
		   		$rmk .= Resources::img("edit.png",array("style"=>"border:2px green solid;margin:2px;","title"=>"Edit Claim","onclick"=>"editrejectedclaim(\"".$elem->rec."\",this)"));
			}



           if($elem->rmks==='3'&&($_SESSION['userlevel']=='5')){	
           		$rmk=Resources::img("unreject.png",  array("style"=>"border:2px red solid;margin:2px;","title"=>"Approve/ Unreject","id"=>"rmk_".$elem->rec."","onclick"=>"editRemarks(this,4,$elem->randomID);"));
			   $rmk .=" <span id='rmk_".$elem->rec."_span' style='cursor:pointer;' onclick='editRemarks(this,3,$elem->randomID)'>[Add Comments]</span>";
		   }
		   
 			if($elem->rmks==='1'||$elem->rmks==='3'||$elem->rmks==='5'){
           		$rmk.= " ".Resources::img("info.png",array("Title"=>"View Notes","style"=>"cursor:pointer;","onclick"=>"viewNotes(\"".$elem->rec."\",this)"));
           	}
           ...
           if($elem->rmks==='7'){
           		$rmk = Resources::img("pay.png",array("title"=>"Paid"));
           }
           ...
           //Start Receipt Control
			...
			$delRef ="";
            if(!empty($elem->rct)){
            	$delRct=Resources::img("diskdel.png",array("title"=>"Delete Receipt","style"=>"border:2px red solid;margin:2px;","onclick"=>"delReceipt(\"".$elem->rct."\",\"".$elem->cluster."\",\"".$elem->rec."\",\"".$elem->randomID."\",\"rct\");"));
 			...
 			if(!empty($elem->refNo)){
       			$delRef=Resources::img("diskdel.png",array("title"=>"Delete Receipt","style"=>"border:2px red solid;margin:2px;","onclick"=>"delReceipt(\"".$elem->refNo."\",\"".$elem->cluster."\",\"".$elem->rec."\",\"".$elem->randomID."\",\"sup\");"));
            	...
            	$refNo = "<form method='POST' action='' id='frm-".$elem->rec."' enctype='multipart/form-data'>"
                ."<input type='file' name='refNo' id='refNo' onchange='uploadRef(\"".$elem->rec."\",\"frm-".$elem->rec."\");'/>"
                ."<input type='hidden' name='pNo' value='".$elem->proNo."'/>"
                ."<input type='hidden' name='clst' value='".$elem->cluster."'/>"
                ."<input type='hidden' name='childNo' value='".$elem->childNo."'/>"
                ."<input type='hidden' name='rec' value='".$elem->rec."'/>"
	...
	$stmp = date("Y-m-d H:m:s",strtotime($stmprw)+32400); 
      
	  $style="";
       if($elem->reinstatementdate!=='0000-00-00' && ($elem->rmks==='1' || $elem->rmks==='3')){
       		$style= "style='background-color:green;'";
       	}elseif($elem->reinstatementdate!=='0000-00-00' && $elem->rmks==='2'){
       		$style= "style='background-color:orange;'";
       	}	
           //End Receipt Control  
      		echo "<tr ".$style." id='rw_".$elem->rec."_".$elem->childNo."'><td><input type='checkbox' id='chk_".$elem->rec."' class='chks'/>".$waive."</td><td>{$elem->proNo}</td><td>{$elem->cluster}</td><td>{$elem->childNo}</td><td>{$elem->childName}</td><td>{$elem->treatDate}</td><td>{$elem->diagnosis}</td><td>".number_format($elem->totAmt,0)."</td><td>".number_format($elem->careContr,0)."</td><td>{$elem->nhif}</td><td>".number_format($elem->amtReim)."</td><td>{$elem->facName}</td><td>{$elem->facClass}</td><td>{$elem->type}</td><td>{$elem->date}</td><td>{$elem->vnum}</td><td id='rct_".$elem->rec."'>{$rct}</td><td id='sup_".$elem->rec."'>{$refNo}</td><td>{$elem->claimCnt}</td><td id='rmk_td_".$elem->rec."'>".$rmk."</td><td>".$stmp."</td></tr>";
