...
<body>
    <form method="post" action="<?php echo url_tag("Settings/userslist")?>">
    ...
    <table class="designerTable" style="border-collapse: collapse;">
		...
		<?php 
			...
			{
				echo '<tr><td><a href='.url_tag("Settings/userslist/userlevel/1/cname/".$record->cname."").'>'.$record->fname.'</a></td><td>'.$record->lname.'</td><td>'.$record->cname.'</td><td>'.$record->userlevel.'</td></tr>';
			}
