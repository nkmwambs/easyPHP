...
function createclass(){
    ...
    if(academicyear.length!==4||isNaN(parseInt(academicyear)/2)){
    	alert("Academic year must be a 4 characters long integer!");
    	document.getElementById("academicyear").style.backgroundColor='red';
    	exit;
    }
    
    var frmData = new FormData();
    frmData.append("classname",classname);
    frmData.append("gradelevelid",gradelevel);
...
function togglebutton(){
	...
	if(chk===false){
		document.getElementById('delstudent').value='Delete';
	}else{
		document.getElementById('delstudent').value='Add';
	}
}
function studentinclasssearch(){
	var st  = document.getElementById('searchstudent').value;
	xmlhttp.onreadystatechange=function() {
            if(xmlhttp.readyState!==4){
                document.getElementById('overlay').style.display='block';
                document.getElementById('overlay').innerHTML='<img id="loading" src= "'+path+'/system/images/loading.gif"/>';
...
function searchclass(){
	xmlhttp.onreadystatechange=function() {
            ...
            if (xmlhttp.readyState===4 && xmlhttp.status===200) {
                ...
                //var obj = JSON.parse(xmlhttp.responseText);
                //document.getElementById('smart_rst').appendChild(create_table(obj));
                document.getElementById('smart_rst').innerHTML=xmlhttp.responseText;
   	...
   	var classname  = document.getElementById('classname').value;
   	var gradelevelid  = document.getElementById('gradelevel').value;
   	var academicyear  = document.getElementById('academicyear').value;
...
function showStudents(elem){
	...
	var classid = row.childNodes[0].innerHTML;
